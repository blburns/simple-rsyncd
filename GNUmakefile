...
# Requires GNU Make (else ifeq and conditional directives are not portable)
ifndef MAKE_VERSION
$(error This build requires GNU Make. On FreeBSD run: gmake)
endif
...
ifeq ($(PLATFORM),windows)
	...
	cd $(BUILD_DIR) && cmake .. -DCMAKE_BUILD_TYPE=Debug && cmake --build . -j$(PARALLEL_JOBS)
endif
...
ifeq ($(PLATFORM),macos)
	...
	pkg install -y gmake cmake openssl jsoncpp pkgconf
else ifeq ($(PLATFORM),windows)
...
ifeq ($(PLATFORM),macos)
	...
	pkg install -y gmake cmake cppcheck py311-pip
	pip3 install bandit semgrep
else ifeq ($(PLATFORM),windows)
...
ifeq ($(PLATFORM),macos)
	...
	@if exist scripts\build-windows.bat ( \
		...
		echo "  sc create SimpleRSyncDaemon binPath= \"$(INSTALL_PREFIX)\\bin\\$(PROJECT_NAME).exe\""; \
...
help:
	@echo "Simple RSync Daemon - A lightweight and secure rsync server - Main Help"
	@echo "=================================="
	@echo ""
...
help-all:
	@echo "Simple RSync Daemon - A lightweight and secure rsync server - All Available Targets"
	@echo "=============================================="
	@echo ""
	@echo "Essential targets:"
	@echo "  all              - Build the project (default)"
...
ifeq ($(PLATFORM),macos)
	...
	@sc config SimpleRSyncDaemon start= auto
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc config SimpleRSyncDaemon start= disabled
endif
...
ifeq ($(PLATFORM),macos)
	...
	@sc query SimpleRSyncDaemon >nul 2>&1 && ( \
		sc stop SimpleRSyncDaemon; \
