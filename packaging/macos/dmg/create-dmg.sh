#!/bin/bash
# Create DMG installer for simple-rsyncd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-rsyncd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-rsyncd.app"
PKG_NAME="$simple-rsyncd.pkg"
...
if [ -f "../assets/icons/$simple-rsyncd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-rsyncd.icns"
fi
