...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-rsyncd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-rsyncd is a modern, secure implementation of the RSync protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id rsyncddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" rsyncddev
    fi
fi
