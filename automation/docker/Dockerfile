...
RUN useradd -r -s /bin/false simple-rsyncd

# Create necessary directories
RUN mkdir -p /etc/simple-rsyncd \
    /var/lib/simple-rsyncd \
    /var/log/simple-rsyncd \
    /var/run/simple-rsyncd

# Copy binary from builder
COPY --from=ubuntu-builder /app/build/bin/simple-rsyncd-x86_64 /usr/local/bin/simple-rsyncd
COPY --from=ubuntu-builder /app/config/ /etc/simple-rsyncd/

# Set ownership
RUN chown -R simple-rsyncd:simple-rsyncd /etc/simple-rsyncd \
    /var/lib/simple-rsyncd \
    /var/log/simple-rsyncd \
    /var/run/simple-rsyncd

# Expose rsync daemon port (typically 873)
EXPOSE 873/tcp
...
# Default command
CMD ["simple-rsyncd", "--config", "/etc/simple-rsyncd/rsyncd.conf"]
